...
public class LogicalPlan {
     ...
     * to {@link Catalog#getDatabaseFile(int)}).
     ...
     *                          added via {@link #addScan} or if field is ambiguous
     *                          (e.g., two
     ...
     *                   the name is ambiguous or unknown, a ParsingException will
     *                   be
     *                   thrown.
     ...
     *                          or is not in one of the tables added via
     *                          {@link #addScan}
     */
     ...
     *                   of the subquery is the first field in the result set of the
     *                   query
     * @param pred       The join predicate.
     * @throws ParsingException if either of the fields is ambiguous,
     *                          or is not in one of the tables added via
     ...
     * Fields are output by the query such that the rightmost field is the first
     * added via addProjectField.
     * 
     ...
     *         parameter is already qualified
            ...
            try {
                ...
                if (tableName == null) {
                    ...
                    throw new ParsingException("Field " + name
                            + " appears in multiple tables; disambiguate by referring to it as tablename." + name);
                }
     ...
     * {@link OpIterator}. Attempts to
     * find the optimal plan by using {@link JoinOptimizer#orderJoins} to order the
     * joins in the plan.
     * 
     * @param t              The transaction that the returned OpIterator will run
     *                       as a part of
     * @param baseTableStats a HashMap providing a {@link TableStats}
     *                       object for each table used in the LogicalPlan. This
     *                       should
     *                       have one entry for each table referenced by the plan,
     *                       not one
     *                       entry for each table alias (so a table t aliases as t1
     *                       and
     *                       t2 would have just one entry with key 't' in this
     *                       HashMap).
    ...
    public OpIterator physicalPlan(TransactionId t, Map<String, TableStats> baseTableStats, boolean explain)
        ...
        for (LogicalFilterNode lf : filters) {
            ...
            double sel = s.estimateSelectivity(subplan.getTupleDesc().fieldNameToIndex(lf.fieldQuantifiedName), lf.p,
                    f);
